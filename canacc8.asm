...
; RAM storage
  ...
  Fsr_temp2L
  Fsr_temp2H
  Fsr_snd1L   ;temp FSR save in TX_frame
  Fsr_snd1H
  Fsr_snd0L
  Fsr_snd0H
...
PRCKSUM     equ MAN_NO+MINOR_VER+MODULE_ID+EVT_NUM+EVperEVT+NV_NUM+MAJOR_VER+NODEFLGS+CPU_TYPE+PB_CAN+HIGH myName+LOW myName+HIGH RESET_VECT+LOW RESET_VECT+PRMCOUNT+CPUM_MICROCHIP+BETA
...
loadadr
    ...
    ORG META_PARAM
prmcnt
    dw  PRMCOUNT    ; Number of parameters implemented

nodenam
    dw  myName      ; Pointer to module type name
    dw  0 ; Top 2 bytes of 32 bit address not used

cksum       
    dw  PRCKSUM     ; Checksum of parameters
...
DoTimer  macro  timer, timeout_bit, timer_copy
    ...
    bsf     Timout, timeout_bit  ;set bits in Timout if it needs to go off
    movff   timer_copy, timer
...
FeedbackCheck  macro  send_feedback

    rlncf   Rollfb,F
    movf    Rollfb,W
    andwf   EVtemp,W        ;is this output enabled?
    bnz     send_feedback   ;yes, send feedback event

    endm

fb2   
    FeedbackCheck fb_ev2

fb2b  
    incf    Op_fb

fb3   
    FeedbackCheck fb_ev3

fb3b  
    incf    Op_fb

fb4   
    FeedbackCheck fb_ev4

fb4b  
    incf    Op_fb

fb5   
    FeedbackCheck fb_ev5

fb5b  
    incf    Op_fb

fb6   
    FeedbackCheck fb_ev6

fb6b  
    incf    Op_fb

fb7   
    FeedbackCheck fb_ev7

fb7b  
    incf    Op_fb

fb8   
    FeedbackCheck fb_ev8
...
FeedbackSend  macro  event, no_feedback, feedback_done
              local  fb_on, fb_off

    btfss   event,7    ;any FB event?
    bra     no_feedback    ;no
    ...
    btfsc   event,6    ;polarity reversed?
    bra     fb_on
    bra     fb_off
...
fb_ev1
    FeedbackSend  E1, fb1b, fb_out

fb_ev2  
    FeedbackSend  E2, fb2b, fb_out

fb_ev3  
    FeedbackSend  E3, fb3b, fb_out

fb_ev4  
    FeedbackSend  E4, fb4b, fb_out

fb_ev5  
    FeedbackSend  E5, fb5b, fb_out

fb_ev6  
    FeedbackSend  E6, fb6b, fb_out

fb_ev7  
    FeedbackSend  E7, fb7b, fb_out

fb_ev8  
    FeedbackSend  E8, fb8b, lpend


fb_out  
    incf    Op_fb   ;end except for last one
    movf    Tmr3h,W
...
; When jump is too far for direct bra branch via these gotos
notNNx  goto notNN
go_on_x goto go_on
...
; When jump is too long for direct bra branch via these gotoss
setNVx  goto setNV
readNVx goto readNV
readENx goto readEN
unsetx  goto unset
...
;********************************************************************
paraerr 
    movlw 3       ;error not in setup mode
    goto  errmsg
...
;*************************************************************************
setup 
    lfsr  FSR0, 0     ; clear 1st page of ram
...
seten_f 
    call  evcopy      ;initialise EVs etc to RAM
    movlw OPC_ACON      ;startup event is ON
    movwf ev_opc
...
seten 
    ...
    movwf INTCON      ;enable interrupts
    bcf   LEDY_OUT
    bsf   LEDG_OUT     ;RUN LED on. Green for SLiM
    goto  main_loop
...
poll  
    movff FSR0L,Fsr_fb0L  ;in case a poll during a flashing
...
polbak  
    movff Fsr_fb0L,FSR0L  ;in case a poll during a flashing
...
ev_ram  
    movlw OLD_EVT_NUM    ;now copy original EVs to RAM
    movwf Count     ;number of ENs allowed
    bcf   STATUS,C
...
enclear 
    movlw OLD_EVT_NUM * 6 + 2    ;number of locations in EEPROM
    movwf Count
    movlw LOW ENindex
    movwf EEADR
...
;************************************************************
getop 
    movlw B'00010011'   ;get DIP switch setting for output
    andwf PORTB,W
    movwf Temp
    movwf Temp1
    rrncf Temp1,F
...
self_en1  
    btfsc PIR2,TMR3IF   ;setup timer out?
    bra   en_done
...
short1  
    clrf  ev0         ;for short events, clear ev0 and ev1
    clrf  ev1
...
dn_teach    
    btfss EVdata,7    ;is it a DN setting?
    return      ;no
...
fbev1 
    lfsr  FSR0,Tx1d1  ;load event into buffer
    movlw LOW ENstart
    movwf EEADR
...
fb_loop 
    ...
    goto  TX_with_NN    ;add NN



f_long
    goto  TX_data   ;don't change NN
